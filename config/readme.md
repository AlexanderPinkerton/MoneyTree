...
KingStack uses **@moneytree/config**, a powerful **TypeScript-based configuration system** that provides type safety, computed values, centralized configuration, and automatic generation of both `.env` files and config files.
...
> **Note:** The core logic now lives in the `@moneytree/config` package. This directory only contains your configuration data.
...
Each environment has its own customized values file. These files import `defineValues` from `@moneytree/config`.
...
This system is powered by the `@moneytree/config` package.
- **CLI**: `king-config`
