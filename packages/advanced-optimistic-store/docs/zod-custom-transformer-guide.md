...
This guide shows how to create custom transformers using Zod for type-safe data validation and transformation in the `@moneytree/advanced-optimistic-store`.
...
```typescript
import { z } from 'zod';
import type { DataTransformer } from '@moneytree/advanced-optimistic-store';
